...
from preproc import get_image_pages
...
for folder in bookFolders:
   ...
   if bookGender == 'M':
      print('O livro ' + folder + ' e menino')
      shutil.move(folder,maleDir)
   if bookGender == 'F':
      print('O livro ' + folder + ' e menina')
...
for book in bookFolders:
   ...
   for page in pages:
      ...
      img = get_image_pages(img)
      cv2.imwrite(page + "fixed 1" + ".png",img[0])
      cv2.imwrite(page + "fixed 2" + ".png",img[1])
   os.chdir("..")
